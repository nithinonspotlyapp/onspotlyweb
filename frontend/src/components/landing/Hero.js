...
import { Zap, MapPin, Camera, ArrowRight, Home } from "lucide-react";
    ...
    <div className="relative w-full max-w-[460px] aspect-square mx-auto">
      <svg viewBox="0 0 400 400" className="w-full h-full">
        ...
        <g>
          ...
          <text x="200" y="231" textAnchor="middle" fill="#A78BFA" fontSize="7.5" fontWeight="700" fontFamily="Manrope, sans-serif">
            You
          </text>
        </g>
      </svg>
      ...
      {/* Overlay label */}
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-full bg-white/5 backdrop-blur-sm border border-white/10 text-[10px] text-zinc-400 tracking-wider uppercase font-medium whitespace-nowrap">
        6 creators nearby
      </div>
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 w-full relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div>
            ...
            {/* Main Headline - THE highlighted text */}
            <motion.h1
            ...
            <motion.p
              ...
              className="text-base md:text-lg text-zinc-400 leading-relaxed mb-4 max-w-lg italic"
            >
            ...
            <motion.p
              ...
              className="text-sm text-zinc-500 leading-relaxed mb-10 max-w-md"
            >
            ...
            >
              ...
              <button
                ...
                className="px-8 py-3.5 rounded-full text-sm font-semibold text-white border border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/20 transition-colors duration-200 flex items-center justify-center gap-2"
              >
                <Camera size={16} />
                Become a Shooter
              </button>
              ...
              {steps.map((step, i) => (
                <div key={i} className="flex items-center gap-3 md:gap-4">
                  <motion.div
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-full ${step.bg} border ${step.border} transition-colors duration-300`}
                  >
                    <step.icon size={16} className={step.color} />
                    <span className={`text-sm md:text-base font-semibold ${activeStep >= i ? "text-white" : "text-zinc-600"} transition-colors duration-300`}>
                      {step.label}
                    </span>
                  </motion.div>
                  {i < 2 && (
                    <motion.div
                      ...
                      className="w-6 md:w-8 h-0.5 bg-gradient-to-r from-zinc-600 to-zinc-700 origin-left"
                    />
                  )}
                </div>
              ))}
